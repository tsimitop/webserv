...
{
	std::filesystem::path current_uploads_dir = this->current_server_.uploads_dir_;
	std::map<int, std::filesystem::path> available_errors = this->current_server_.errors;
	// std::string temp_filename = decodingHexToAscii(this->filename_);
	
	std::filesystem::path	path_of_file_to_delete = current_uploads_dir / this->filename_;
